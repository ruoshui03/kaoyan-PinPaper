...
考研数学智能组卷与错题攻坚系统。按真题结构从题库中组卷、按科目独立管理错题、导出考场级 PDF，帮助你把有限的刷题时间集中到薄弱考点上。

**内置三本题库**（数一 / 数二 / 数三各自独立，可多选汇聚为组卷题池）：

| 题库 | 数一 | 数二 | 数三 | 说明 |
| --- | --- | --- | --- | --- |
| 李林《880》 | 1121 | 921 | 1019 | 按 21 章 × 基础 / 综合 / 拓展篇组织 |
| 历年真题 2010–2026 | 385 | 385 | 385 | 按年份组织，难度经逐题判定 |
| 张宇《1000题》 | 1296 | 945 | 1160 | 按 基础篇 / 强化篇 / 综合篇（测试卷）× 高数 / 线代 / 概率组织 |
...
- 🎯 **智能组卷**：
  ...
  - **真题章节分布参考**：学习 2010–2026 历年真题各章考频，组卷时按强度（可调）软性偏向真题热点章节，保留随机性、不排除任何章；
  - **避免重复抽题**：记录你抽过的新题，后续组卷自动跳过，全书抽完后自动重置；错题重练不受影响。
- 📕 **错题靶向重练**：
  - **按题号导入**：刷完一章后直接输入做错的原书题号批量标记，输入框按各书的编号方式给好范围提示；
  - **优先重练**：组卷默认以待练错题为题源，直接生成错题卷；
  - **顽固错题**：做错次数累计，反复做错的题重点标记、集中攻坚；
  - **错题本导出 PDF**：按书籍 / 状态 / 章节 / 题型筛选后，把筛选结果导成三种版式之一（详见下文）。
- 📈 **考点雷达**：
  - **覆盖进度**：目标章节的组卷覆盖率与进度条；
  - **顽固清单**：本科目高频顽固错题按做错次数排序汇总；
  - **分科画像**：按高数 / 线代 / 概率分类展示各章覆盖与错题分布。
- 📑 **三版式 PDF 导出**（组卷与错题本均可导出）：
  - **真题版**：紧密排版，无答案，需自备答题卡；
  - **A4 做题本**：全卷留白演算空间，适合 iPad 导题与打印刷题；
  - **详细解析版**：参考答案与分步解析，每题标注出处书籍、难度、章节与考点标签；
  - 为保证公式排版清晰，PDF 需点「生成」按钮后现场排版，约十几秒。
...
- 🔗 **链接即存档（无后端跨设备）**：错题状态与上次生成的试卷都编码进页面网址，收藏或复制该链接即可在其他设备恢复——无需登录、无需数据库。
  - **错题进度**：三本题库的错题分别压缩进网址（每本约 30 字符），做错次数分待练 / 顽固两档保真；
  - **抽题记录**：「抽过的题」同样记进网址，跨设备延续「避免重复抽题」；
  - **上次的卷**：记住上次生成的是哪几道题（5-3-3 / 自定义 / 3 套联考通用），做完后凭链接回来查阅答案；
  - **书籍选择**：勾选了哪几本参考书也记在网址里，下次打开自动恢复上次的选择；
...
| 🏷️ **题库逐题标错** | 选书籍与章节浏览题目，逐题点选或按题号批量标记错题、增减做错次数 |
| 📕 **我的错题本** | 全科错题一览，按书籍 / 状态 / 章节 / 题型筛选，可导出错题本 PDF 与 JSON 备份 |
...
├── 题库资料/                  # 三本题库 × 数一 / 数二 / 数三
│   ├── 880数学X/              # 李林 880（题面与元数据分离，按章对齐）
│   │   ├── metadata/*.json    #   题目元数据（章节、难度、标签、答案）
│   │   └── problems/*.md      #   题面正文（LaTeX 公式）
│   ├── 真题数学X/             # 历年真题 2010–2026（自包含，按年份分文件）
│   ├── 1000题数学X/           # 张宇 1000题（自包含，按 领域-篇 分文件）
│   │   ├── metadata/*.json    #   题目元数据与题面、选项
│   │   └── problems/          #   人读版 md 与题图（images/）
│   └── 真题章节分布.json      # 历年真题各章×题型考频权重（组卷软偏好用）
